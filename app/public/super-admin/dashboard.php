...
<body class="bg-gray-50 h-screen">
    <nav class="bg-white shadow-sm border-b px-6 py-3 flex justify-between items-center">
        ...
        <div class="flex items-center space-x-4">
            <a href="profile.php" class="text-sm text-blue-600 hover:underline">Profile & Security</a>
    ...
    <div class="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-8 border border-gray-200">
            ...
            <form action="add_company.php" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <input type="number" step="0.01" name="tax" placeholder="Tax % (e.g., 15.00)" required class="border p-2 rounded focus:ring-blue-500">
        ...
        <!-- Super Admin Team -->
        <div class="bg-white rounded-lg shadow p-6 mb-8 border border-gray-200">
            <h2 class="text-xl font-semibold mb-4 text-blue-800">Super Admin Team</h2>
            <form action="add_sa_user.php" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <input type="text" name="name" placeholder="Team Member Name" required class="border p-2 rounded">
                <input type="email" name="email" placeholder="Team Member Email" required class="border p-2 rounded">
                <input type="password" name="password" placeholder="Temporary Password" required class="border p-2 rounded">
                <button type="submit" class="bg-gray-800 text-white p-2 rounded hover:bg-gray-900 font-bold md:col-span-3 w-fit">Grant Super Admin Access</button>
            </form>
            <table class="w-full text-left border-collapse">
                ...
                <tbody>
                    <?php foreach ($sa_users as $u): ?>
                        <tr class="border-b">
                            <td class="p-2"><?= sanitize($u['name']) ?></td>
                            <td class="p-2 text-blue-600"><?= sanitize($u['email']) ?></td>
                            <td class="p-2 text-sm text-gray-500"><?= date('M d, Y', strtotime($u['created_at'])) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden border border-gray-200">
            <div class="p-4 border-b bg-gray-50"><h2 class="text-xl font-semibold">Active Restaurants (<?= count($companies) ?>)</h2></div>
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-100 text-gray-600 uppercase text-sm">
                        <th class="p-3">ID</th>
