...
<body class="bg-gray-50 h-screen">
    ...
    <div class="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-8 border border-gray-200">
            ...
            <form action="add_company.php" method="POST" class="space-y-6">
                <div><div class="section-title">Basic Information</div><div class="grid grid-cols-1 md:grid-cols-3 gap-4"><input type="text" name="name" placeholder="Restaurant Name *" required class="border p-2 rounded"><input type="text" name="slug" placeholder="URL Slug *" required class="border p-2 rounded"><input type="text" name="phone" placeholder="Phone Number" class="border p-2 rounded"></div></div>
                <div><div class="section-title">Network & Access (Advanced)</div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="text-xs text-gray-500 block mb-1">Custom Domain or Subdomain (Optional)</label>
                            <input type="text" name="custom_domain" placeholder="e.g., orders.joespizza.com" class="border p-2 rounded w-full">
                            <p class="text-xs text-red-400 mt-1">* DNS A-Record must point to YOUR VPS IP before saving.</p>
                        </div>
                        <div class="flex items-end text-sm text-gray-500 bg-gray-50 p-2 rounded">
                            If left blank, system will auto-assign an isolated Port (e.g., :8081, :8082).
                        </div>
                    </div>
                </div>
                ...
                <div><div class="section-title">Mobile Bank Transfer Numbers</div><div id="mobile-numbers-container" class="space-y-2 mb-2"><div class="flex gap-2"><input type="text" name="mobile_numbers[]" placeholder="Bank Number" class="flex-1 border p-2 rounded"><button type="button" onclick="this.parentElement.remove()" class="bg-red-100 text-red-700 px-3 rounded hover:bg-red-200 text-sm font-bold">X</button></div></div><button type="button" onclick="addMobileField()" class="text-sm bg-gray-100 text-gray-700 px-3 py-1 rounded hover:bg-gray-200 font-semibold">+ Add Bank</button></div>
                <div><div class="section-title text-red-700">Admin Credentials</div><div class="grid grid-cols-1 md:grid-cols-3 gap-4"><input type="text" name="admin_name" placeholder="Admin Name *" required class="border p-2 rounded border-red-200"><input type="email" name="admin_email" placeholder="Admin Email *" required class="border p-2 rounded border-red-200"><input type="password" name="admin_password" placeholder="Admin Password *" required minlength="8" class="border p-2 rounded border-red-200"></div></div>
                <div><div class="section-title">Appearance</div><div class="grid grid-cols-1 md:grid-cols-2 gap-4"><select name="theme" class="border p-2 rounded"><option value="midnight">Midnight Luxury</option><option value="garden">Garden Fresh</option><option value="classic">Classic Diner</option><option value="rustic">Rustic Bakery</option></select><button type="submit" class="bg-blue-600 text-white p-2 rounded hover:bg-blue-700 font-bold">Provision Server & Save</button></div></div>
            </form>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            <div class="lg:col-span-1 bg-white rounded-lg shadow p-4 border">
                ...
                <form action="add_sa_user.php" method="POST" class="space-y-2 mb-4"><input type="text" name="name" placeholder="Name" required class="w-full border p-2 rounded text-sm"><input type="email" name="email" placeholder="Email" required class="w-full border p-2 rounded text-sm"><input type="password" name="password" placeholder="Password" required class="w-full border p-2 rounded text-sm"><button type="submit" class="w-full bg-gray-800 text-white p-2 rounded text-sm hover:bg-gray-900">Add</button></form>
                <?php foreach ($sa_users as $u): ?><div class="text-sm border-b py-1"><span class="font-semibold"><?= sanitize($u['name']) ?></span> <span class="text-gray-400 text-xs block"><?= sanitize($u['email']) ?></span></div><?php endforeach; ?>
            </div>

            <div class="lg:col-span-3 bg-white rounded-lg shadow overflow-hidden border">
                <div class="p-4 border-b bg-gray-50"><h2 class="text-lg font-semibold">Active Instances (<?= count($companies) ?>)</h2></div>
                <table class="w-full text-left border-collapse text-sm">
                    <thead class="bg-gray-100 text-gray-600 uppercase text-xs"><tr><th class="p-3">Client</th><th class="p-3">Access URL</th><th class="p-3">Admin</th><th class="p-3">Actions</th></tr></thead>
                    <tbody>
                        <?php foreach ($companies as $c): 
                            $adm = $pdo->prepare("SELECT email FROM users WHERE company_id = ? AND role='company_admin' LIMIT 1"); $adm->execute([$c['id']]); $admin_email = $adm->fetchColumn();
                        ...
                        ?>
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="p-3"><a href="<?= $access_url ?>" target="_blank" class="text-blue-600 hover:underline font-mono text-xs"><?= $access_url ?></a></td>
                                <td class="p-3 text-gray-500 text-xs"><?= sanitize($admin_email ?? 'None') ?></td>
                                <td class="p-3 space-x-2">
                                    <a href="delete_company.php?id=<?= $c['id'] ?>" class="text-red-600 hover:underline text-xs font-bold" onclick="return confirm('Delete this entire instance?')">Destroy</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
    ...
    <script>function addMobileField(){const c=document.getElementById('mobile-numbers-container');const d=document.createElement('div');d.className='flex gap-2';d.innerHTML=`<input type="text" name="mobile_numbers[]" placeholder="Another number..." class="flex-1 border p-2 rounded"><button type="button" onclick="this.parentElement.remove()" class="bg-red-100 text-red-700 px-3 rounded hover:bg-red-200 text-sm font-bold">X</button>`;c.appendChild(d);}</script>
</body>
</html>
